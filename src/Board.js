...
export default class Board extends React.Component {
  ...
  componentDidMount() {
    ...
    this.dragula.on('drop', (element, target) => {
      this.handleDrop(element, target);
    });
  }
  ...
  handleDrop(element, target) {
    ...
    const statusForLane = [
      {
        element: this.swimlanes.backlog.current,
        status: 'backlog',
        stateKey: 'backlog',
      },
      {
        element: this.swimlanes.inProgress.current,
        status: 'in-progress',
        stateKey: 'inProgress',
      },
      {
        element: this.swimlanes.complete.current,
        status: 'complete',
        stateKey: 'complete',
      },
    ];
    ...
    statusForLane.forEach(lane => {
      Array.from(lane.element.children).forEach(card => {
        const id = card.getAttribute('data-id');
        const client = allClients.find(item => item.id === id);

        if (client) {
          newClients[lane.stateKey].push({
            ...client,
            status: lane.status,
          });
        }
